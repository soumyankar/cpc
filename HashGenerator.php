...
class HashGenerator
{
     ...
     */
    public function __construct($salt = '', $min_hash_length = 10, $alphabet = '')
    {
        ...
        if (strlen($this->_alphabet) < self::MIN_ALPHABET_LENGTH) {
            throw new \DomainException(sprintf(self::E_ALPHABET_LENGTH, self::MIN_ALPHABET_LENGTH));
        }

        if (is_int(strpos($this->_alphabet, ' '))) {
            throw new \DomainException(self::E_ALPHABET_SPACE);
        }
